    ...
    <div class="px-4 py-5 sm:p-6">
        ...
        <div class="mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Edit Customer Segment</h1>
                    <p class="mt-1 text-sm text-gray-500">Update segment details and pricing preferences</p>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ route('customer-segments.show', $customerSegment) }}"
        ...
        <!-- Form -->
        <form method="POST" action="{{ route('customer-segments.update', $customerSegment) }}" x-data="segmentForm">
            @csrf
            @method('PATCH')
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text"
                               id="name"
                               name="name"
                               value="{{ old('name', $customerSegment->name) }}"
                               required
                    ...
                    <div>
                        ...
                        <input type="number"
                               id="sort_order"
                               name="sort_order"
                               value="{{ old('sort_order', $customerSegment->sort_order) }}"
                               min="0"
                               required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        @error('sort_order')
                ...
                <div>
                    ...
                    <textarea id="description"
                              name="description"
                              rows="3"
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $customerSegment->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            <input type="number"
                                   ...
                                   value="{{ old('default_discount_percentage', $customerSegment->default_discount_percentage) }}"
                                   step="0.01"
                                   min="0"
                                   max="100"
                                   required
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex items-center space-x-3">
                            <input type="color"
                                   id="color"
                                   name="color"
                                   value="{{ old('color', $customerSegment->color) }}"
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox"
                               name="is_active"
                               value="1"
                               {{ old('is_active', $customerSegment->is_active) ? 'checked' : '' }}
                ...
                @if($customerSegment->customers()->count() > 0)
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <h3 class="text-sm font-medium text-yellow-800">
                                    Impact on {{ $customerSegment->customers()->count() }} Customers
                                </h3>
                                <p class="mt-2 text-sm text-yellow-700">
                                    Changes to this segment will affect pricing for all assigned customers.
                                    Existing quotations and invoices will not be updated automatically.
                                </p>
                            </div>
                        </div>
                    </div>
...
function segmentForm() {
    return {
        selectedColor: '{{ old('color', $customerSegment->color) }}'
    }
}
</script>
@endsection