...
<div x-data="testimonialCarousel()" 
     x-init="init({{ $autoplay ? 'true' : 'false' }}, {{ $interval }})"
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
        ...
        <p class="text-blue-100 text-sm mt-1">{{ $testimonials->count() }} verified reviews</p>
    </div>
        ...
        <div x-show="activeSlide === {{ $index }}"
            ...
            <div class="text-center">
                <div class="flex items-center justify-center space-x-4 mb-3">
                    @if($testimonial->customer_photo_path)
                        <img src="{{ Storage::url($testimonial->customer_photo_path) }}" 
                    ...
                    <div class="text-left">
                        <div class="font-semibold text-gray-900">{{ $testimonial->customer_name }}</div>
                        @if($testimonial->company_name)
                            <div class="text-sm text-gray-600">{{ $testimonial->company_name }}</div>
                        @endif
                    </div>
                </div>
                ...
                @if($testimonial->service_provided)
        ...
        <div class="flex space-x-2">
            @foreach($testimonials as $index => $testimonial)
                <button @click="activeSlide = {{ $index }}" 
                        :class="activeSlide === {{ $index }} ? 'bg-blue-600' : 'bg-white/60'"
