...
function invoiceBuilder() {
    return {
        ...
        selectedCustomer: {
            ...
            name: @json($invoice->customer_name ?? ''),
            email: @json($invoice->customer_email ?? ''),
            phone: @json($invoice->customer_phone ?? ''),
            company_name: @json($invoice->customer_company ?? ''),
            full_address: @json($invoice->customer_address ?? ''),
            address: @json($invoice->customer_address ?? ''),
            city: @json($invoice->customer_city ?? ''),
            state: @json($invoice->customer_state ?? ''),
            postal_code: @json($invoice->customer_postal_code ?? ''),
            customer_segment_id: {{ $invoice->customer_segment_id ?? 'null' }}
        },
        customerSegmentName: @json($invoice->customerSegment->name ?? ''),
        ...
        invoiceDateDisplay: '',
        dueDate: @json($invoice->due_date?->format('Y-m-d') ?? date('Y-m-d', strtotime('+30 days'))),
        dueDateDisplay: '',
        ...
        // Content
        notes: @json($invoice->notes ?? 'Thank you for your business!'),
        terms: @json($invoice->terms_conditions ?? 'Payment is due within 30 days. Late payments may incur additional charges.'),
        paymentInstructions: @json($invoice->payment_instructions ?? 'Please make payments to:\n\nCompany: ' . (auth()->user()->company->name ?? "Your Company Name") . '\nBank: Maybank\nAccount: 1234567890\n\nPlease include invoice number in payment reference.'),
        ...
        previewPDF() {
                ...
                if (this.isOwner) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            this.$dispatch('notify', { type: 'success', message: 'Changes saved. Opening PDF preview...' });
                        } else {
                            this.$dispatch('notify', { type: 'error', message: data.message || 'Failed to save changes' });
                        }
                    })
                    .catch(error => {
                        console.error('Preview error:', error);
                        this.$dispatch('notify', { type: 'error', message: 'Failed to save changes' });
                    });
                } else {
                    // Non-owner: just open preview without saving
                    window.open(`/invoices/${this.currentInvoiceId}/preview`, '_blank');
            ...
            // This shouldn't happen in edit mode, but keeping for safety
            this.$dispatch('notify', { type: 'error', message: 'Unable to generate preview' });
        },
