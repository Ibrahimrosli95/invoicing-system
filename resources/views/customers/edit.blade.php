        ...
        <form action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="address" id="address" rows="2"
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('address', $customer->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="customer_segment_id" id="customer_segment_id"
                        ...
                        @foreach($customerSegments as $segment)
                            <option value="{{ $segment->id }}" {{ old('customer_segment_id', $customer->customer_segment_id) == $segment->id ? 'selected' : '' }}>
                                {{ $segment->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-4">
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" name="is_new_customer" id="is_new_customer" value="1"
                               {{ old('is_new_customer', $customer->is_new_customer) ? 'checked' : '' }}
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('notes', $customer->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
